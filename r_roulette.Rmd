...
Let's try the same game of placing 1000 bets, 1000 times.
```{r}
betOnRed <- function(){ 
  games <- sample(Roulette$color, 1000, replace=TRUE, prob=NULL)
  win <-1;
...
```
We can look at quantiles for the winnings to get a sense of the distribution as shown in the histogram above.
```{r}
ms <- as.matrix(quantile (winnings,  p=seq(from = 0, to = 1, by = 0.1)))
  colnames(ms) <- c("winnings")
  print(kable(ms))
```
# Exercise 7, page 13
